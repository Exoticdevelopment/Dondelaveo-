...
def procesar_fecha(fecha_iso):
    ...
    fecha_ddmmyyyy = "/".join(reversed(fecha_iso.split("-")))  # "2026-08-06" -> "06/08/2026"

    conn = get_conn()

    upsert_cine(
        conn, id=int(ID_COMPLEJO), nombre="Izi Movie", ciudad="Cali",
        latitud=None, longitud=None, company_id=ID_COMPLEJO, slug="izi-movie",
        cadena=CADENA,
    )
    conn.commit()

    try:
        cartelera = obtener_cartelera(fecha_ddmmyyyy)
    ...
    for pelicula in peliculas_raw:
        pelicula_id = upsert_pelicula(
            ...
            duracion_min=int(pelicula["peliculas_duracion"]) if pelicula.get("peliculas_duracion") else None,
    ...
    for funcion in funciones_raw:
        ...
        upsert_funcion(
            conn, session_id=session_id, cine_id=int(ID_COMPLEJO), pelicula_id=pelicula_id,
            fecha=fecha_iso, hora=funcion["hora"] + ":00", formato=funcion.get("formato"),
        ...
        try:
            precio = obtener_precio_de_funcion(session_id, codigo_pelicula)
            if precio:
    ...
    print(f"  OK: Izi Movie ({fecha_iso}) — {len(peliculas_raw)} películas, {procesadas} funciones procesadas")
    conn.close()
...
def main():
    ...
    print(f"Procesando Izi Movie para {len(fechas)} días ({fechas[0]} a {fechas[-1]})...\n")
